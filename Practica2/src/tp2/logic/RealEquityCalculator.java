...
/** Calculador de equity por Monte Carlo (real). */
public class RealEquityCalculator {

    /** Calcula equity con manos conocidas (o null si desconocida) y board parcial.
     *  @param names   nombres de jugadores en el mismo orden que hands
     *  @param hands   mano conocida o null (2 cartas tipo "Ah","Kd")
     *  @param board   0..5 cartas visibles (cada una "Qs" etc.)
     *  @param trials  nº de simulaciones
     *  @param seed    semilla (p.ej., basada en estado) para repetibilidad
     */
    public Map<String, Double> calcularEquity(
            List<String> names, List<Hand> hands, List<String> board, int trials, long seed) {

        if (names.size() != hands.size()) throw new IllegalArgumentException("names y hands deben tener misma longitud");
        final int N = names.size();

        // Construir baraja restante excluyendo cartas ya usadas
        ...
        for (int t = 0; t < T; t++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                scores[i] = sc;
                if (sc > best) { best = sc; winners.clear(); winners.add(i); }
                else if (sc == best) { winners.add(i); }
            }
            double share = 1.0 / winners.size(); // split pot
